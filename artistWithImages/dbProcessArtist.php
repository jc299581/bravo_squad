...
{include("imageSave.php");
    $sql = "INSERT INTO Artists (art_name, art_blurb, art_desc, featured, art_img, art_thumbnail) VALUES ('$_REQUEST[art_name]', '$_REQUEST[art_blurb]', '$_REQUEST[art_desc]', '$_REQUEST[featured]', '$newFullName', '$thumbFullName' )";
    echo "<p>Query: " . $sql . "</p>\n<p><strong>";
    if ($dbh->exec($sql))
...
foreach ($dbh->query($sql) as $row) {
    print $row["art_id"] . ' - ' . $row["art_name"] . ' - ' . $row["art_blurb"] . $row["art_desc"] . ' - ' . $row["featured"] . "<br />\n";
}
$dbh = null;
...
<p><a href="artist.php">Return to new Artist</a></p>
<p><a href="manageArtists.php"> Return to manage artists</a></p>
</body>
</html>
